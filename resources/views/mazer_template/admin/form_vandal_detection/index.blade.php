...
@section('title', 'Vandal Detections List')
@section('content')
...
<div class="page-heading">
    <div class="page-title">
        <div class="row">
            <div class="col-12 col-md-6 order-md-1 order-last">
                <h3>@include('mazer_template.layouts.icons.vandal-detection') Vandal Detections <small class="text-muted">List</small></h3>
            </div>
            <div class="col-12 col-md-6 order-md-2 order-first">
                <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('admin.index') }}">Dashboard</a></li>
                        <li class="breadcrumb-item active" aria-current="page">Vandal Detections List</li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <section class="section">
        <div class="card">
            <div class="card-body">
                <button title='Advanced Filter' type="button" class="btn" style='border-radius:12px; background-color:#0000FF; color:white;' data-bs-toggle="modal" data-bs-target="#modalFilterVandalDetection"><i class="bi bi-funnel"
                        style="font-size: 13px; font-weight:bold"></i></button>
                <div class="table-responsive mt-4 mb-4" style="width: 100%;">
                    <table class="table table-hover" id="form_vandal_detection_client_side" style="border-collapse: collapse; width: 100%;">
                        <thead>
                            <tr>
                                <th>No</th>
                                <th>Capture</th>
                        ...
                        {{-- foreach data from $vandal_detection --}}
                        <tbody>
                            @foreach($vandal_detection as $data)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>
                                        <img src="{{ $data['img_url'] }}" alt="{{ $data['file_name_capture_vandal_detection'] }}" width="100" height="100">
                                    </td>
                                    <td>{{ $data['tid'] }}</td>
                                    ...
                                    <td>
                                        ...
                                        <a href="{{ $data['img_url'] }}" download="{{ $data['file_name_capture_vandal_detection'] }}" title="Download Image" type="button" class="btn"
                                                        ...
                                                        <div class="mb-4 mt-2">
                                                            <div class="card">
                                                                <div class="card-body">
                                                                    <div class="row">
                                                                        <div class="col-md">
                                                                            <img src="{{ $data['img_url'] }}" alt="{{ $data['file_name_capture_vandal_detection'] }}" width="100%"
                                                                                height="100%">
                                                                        </div>
                                                                    </div>
                                                                </div>
    ...
    <!-- Modal Filter Vandal Detection -->
    <div class="modal fade" id="modalFilterVandalDetection" role="dialog" aria-labelledby="modalFilterLogAcceptedlLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    {{-- body --}}
                    <form id="form-filter-vandalDetection" method="get" action="{{ route('admin.vandaldetection.index') }}">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group mb-0">
                                    <label for="select2-form-vandalDetection-person" class="form-label text-dark">Is Person</label>
                                    <select id="select2-form-vandalDetection-person" class="form-control select2-form-vandalDetection-person" name="person" style="width: 300px;">
                ...
                <div class="modal-footer">
                    ...
                    <button id="btn-reset-filter-vandalDetection" onClick="changeToLoadingFormResetFilterVandalDetection()" class="btn" style="background-color:#FFA500; color:white;">Reset</button>
                    <button type="" id="submit-filter-vandalDetection" onClick="changeToLoadingFormFilterVandalDetection()" class="btn" style="background-color:#0000FF; color:white;">Filter</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        function changeToLoadingFormFilterVandalDetection() {
            var btn = document.getElementById('submit-filter-vandalDetection');
            ...
            setTimeout(function () {
                ...
                // Submit the form
                submitFormFilterVandalDetection();
            }, 250);
        }

        function submitFormFilterVandalDetection() {
            // Get the form element
            var form = document.getElementById('form-filter-vandalDetection');
    ...
    <script>
        //  btn reset request to null with id='btn-reset-filter-vandalDetection' and reload page
        $('#btn-reset-filter-vandalDetection').on('click', function () {
            $('#select2-form-vandalDetection-tid').val(null).trigger('change');
            $('#input-start-date-time-in-log-accepted').val(null);
            $('#input-end-date-time-in-log-accepted').val(null);
            $('#select2-form-vandalDetection-person').val(null).trigger('change');
        });

        // changeToLoadingFormResetFiltervandalDetection
        function changeToLoadingFormResetFilterVandalDetection() {
            var btn = document.getElementById('btn-reset-filter-vandalDetection');
            btn.innerHTML = '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>';
            btn.disabled = true;
            ...
            setTimeout(function () {
                ...
                // Submit the form
                $('#form-filter-vandalDetection').submit();
            }, 250);
        }
